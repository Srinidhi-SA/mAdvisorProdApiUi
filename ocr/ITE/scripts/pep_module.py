...
class Pep:  # preprocess_extract_pipeline
    ...
    def __init__(self, image_path, crop=False, c2=True, enhance=True):
        ...
        if crop:
            ...
            pass
        self.deskew_optimized(self.image_path)
        if c2:
    ...
    def deskew_optimized(self, image_path):
    ...
    def crop_2parts(self, image):
        ...
        if len(index) >= 1:
            ...
            if sum([1 if crop_index + i in index else 0 for i in range(-5, 6)]) >= 10:
                ...
                optness = {ind: sum([1 if ind + i in index else 0 for i in range(-5, 6)])
                           for ind in range(crop_index - 10,
                                            crop_index + 11) if
            ...
            array_sums = {i: sum(ar)
                          for i, ar in enumerate(img_array) if (i > 50) and (i < height - 50)}
            highest_value = array_sums[max(array_sums, key=array_sums.get)]
            #            print('HV :',highest_value)
            if highest_value >= int(0.9 * (width)):
                filtered_indices = [m for m, n in array_sums.items() if n >= int(0.9 * (width))]
