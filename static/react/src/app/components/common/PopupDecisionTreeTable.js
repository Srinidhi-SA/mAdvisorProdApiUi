    ...
    var tbodyData = table.tableData.map(function(rowData,i){
      if(i != 0){
        ...
        var rows = rowData.map(function(colData,j) {
          if(j == 0){
            return <td key={j} className="cursor">{renderHTML(colData)}</td>;
          }else if(j > 3)
            return  <td class="hidden" key={j}>{colData}</td>
          else 
            return  <td class="text-center" key={j}>{colData}</td>
        });
        return<tr key={i}>{rows}
        {/*<td class="cursor text-center" onClick={that.showDecisionTreePopup.bind(this,rule,path,that.props.tableData.name)}><a data-toggle="modal" class="btn btn-space btn-default btn-round btn-xs"><i class="fa fa-info"></i></a></td>*/}
        </tr>;
      }
    })
    return tbodyData;
