...
export class OcrTable extends React.Component {
  ...
  handleExport = () => {
    ...
    let SelectedDocType = dataList.filter(i => i.slug == checkList)[0].doctype
    ...
    if (SelectedDocType === "pdf") {
      ...
      .then((bytes) => {
        ...
        elm.setAttribute('download', this.state.exportName);
        elm.click() 
        this.setState({ checkAll: false, checkedList: [] })
    }
      )}
                ...
                <ul className="export" >
                  <li className="dropdown">
                    ...
                    <ul className="dropdown-menu" onClick={(e) => this.setState({ exportType: e.target.text.toLowerCase() }, this.handleExport)}>
                      ...
                      {/*<li><a role="tab" value="csv" data-toggle="tab">CSV</a></li>           //Commenting for now*/}
                      <li><a role="tab" value="xml" data-toggle="tab">XML</a></li>
                    </ul>
                  </li>
                </ul>
              </div>
